...
(function() {
    ...
    function addRecent(cmd) {
        ...
        var list = getRecent();
        list = [cmd].concat(list.filter(function(c) {
            return c !== cmd;
        })).slice(0, 30);
        saveRecent(list);
        renderRecent();
    ...
    function renderRecent() {
        ...
        var list = getRecent();
        if (!list.length) {
            box.innerHTML = '<div class="recent-item">暂无记录，生成指令后自动保存。</div>';
            return;
        }
    ...
    document.getElementById("exportTxt").addEventListener("click", function() {
        download("mcbe-history.txt", getRecent().join("\n"));
    });
    document.getElementById("exportJson").addEventListener("click", function() {
        download("mcbe-data.json", JSON.stringify({
            history: getRecent(),
    ...
    document.getElementById("btn-give").addEventListener("click", function() {
        ...
        if (document.getElementById("give-item-lock").checked) comp["minecraft:item_lock"] = {
            mode: "lock_in_slot"
        };
        var place = splitBlocks(document.getElementById("give-placeon").value);
        if (place.length) comp["minecraft:can_place_on"] = {
            blocks: place
        };
        var destroy = splitBlocks(document.getElementById("give-destroy").value);
        if (destroy.length) comp["minecraft:can_destroy"] = {
            blocks: destroy
        };
    ...
    function getFavorites() {
        try {
            return JSON.parse(localStorage.getItem("mcbe_favorite_cmds") || "[]");
        } catch (e) {
            return [];
        }
    }
    ...
    function allTilde(coord) {
        return /^~0?(\.0+)? ~0?(\.0+)? ~0?(\.0+)?$/.test(coord) || /^~~~$/.test(coord.replace(/\s/g, ""));
    }
    var ExecuteConverter = {
        oldToNew: function(line) {
            ...
            var selector = tokens[t].token;
            t++;
            if (t + 2 >= tokens.length) throw new Error("缺少坐标 x y z");
            var coord = tokens[t].token + " " + tokens[t + 1].token + " " + tokens[t + 2].token;
            t += 3;
            var parts = ["as " + selector];
            if (allTilde(coord)) parts.push("at @s");
            else parts.push("positioned " + coord);
            var detectPart = "";
            if (t < tokens.length && tokens[t].token.toLowerCase() === "detect") {
                t++;
                if (t + 2 >= tokens.length) throw new Error("detect 缺少坐标");
                var detectCoord = tokens[t].token + " " + tokens[t + 1].token + " " + tokens[t + 2].token;
                t += 3;
                if (t >= tokens.length) throw new Error("detect 缺少方块id");
                var blockId = tokens[t].token;
                t++;
                var blockData = "-1";
                if (t < tokens.length && /^-?\d+$/.test(tokens[t].token)) {
                    blockData = tokens[t].token;
                    t++;
                }
                detectPart = (blockData === "-1" || blockData === "*") ? ("if block " + detectCoord + " " + blockId) : ("if block " + detectCoord + " " + blockId + " " + blockData);
            }
            var command = tokens.length > t ? tokens.slice(t).map(function(k) {
                return k.token;
            }).join(" ") : "";
        ...
        newToOld: function(line) {
            ...
            while (t < tokens.length) {
                ...
                if (tok === "as" && t + 1 < tokens.length) {
                    selector = tokens[t + 1].token;
                    t += 2;
                } else if (tok === "at" && t + 1 < tokens.length) {
                    hasAt = true;
                    t += 2;
                } else if (tok === "positioned" && t + 1 < tokens.length) {
                    if (tokens[t + 1].token.toLowerCase() === "as") {
                        t += 2;
                        if (t < tokens.length) selector = tokens[t].token;
                        t++;
                    } else {
                        t++;
                        if (t + 2 < tokens.length) {
                            position = tokens[t].token + " " + tokens[t + 1].token + " " + tokens[t + 2].token;
                            t += 3;
                        }
                    }
                ...
                } else if (tok === "run" && t + 1 < tokens.length) {
                    command = tokens.slice(t + 1).map(function(k) {
                        return k.token;
                    }).join(" ");
                    break;
                } else t++;
        ...
        convertAll: function(input, direction) {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                try {
                    out.push(direction === "oldToNew" ? this.oldToNew(line) : this.newToOld(line));
                    ok++;
                } catch (e) {
    ...
    function convertExec(direction) {
        ...
        if (r.errors.length) showMsg(errEl, "转换完成，但存在 " + r.errors.length + " 个错误: " + r.errors[0], true);
            ...
            showMsg(okEl, "转换成功", false);
            addRecent(r.result);
        }
    }
    document.getElementById("convertToNewBtn").addEventListener("click", function() {
        convertExec("oldToNew");
    });
    document.getElementById("convertToOldBtn").addEventListener("click", function() {
        convertExec("newToOld");
    });
    document.getElementById("swapExecute").addEventListener("click", function() {
        var a = document.getElementById("oldToNewInput"),
            b = document.getElementById("newToOldInput");
        var t = a.value;
        a.value = b.value;
        b.value = t;
    });
    ...
    var SITES = [{
            ...
            items: [{
                    name: "InstaFonts",
                    desc: "花体字体生成",
                    url: "https://instafonts.io/"
                },
                {
                    name: "特殊字符",
                    desc: "特殊符号与字符工具",
                    url: "https://teshuzifu.cn/"
                }
            ]
        },
        {
            ...
            items: [{
                ...
                },
                {
                    name: "MinecraftArt",
                    desc: "像素画、投影、地图画生成",
                    url: "https://www.minecraftart.net/zh/"
                },
                {
                    name: "MapartCraft",
                    desc: "地图画 schematic 制作",
                    url: "https://rebane2001.com/mapartcraft/zh-Hans"
                },
                {
                    name: "mcimg",
                    desc: "投影文件下载",
                    url: "https://www.mcimg.com"
                }
            ]
        },
        {
            ...
            items: [{
                ...
                },
                {
                    name: "指令音乐压缩工具",
                    desc: "压缩音乐命令体积",
                    url: "https://backend.appmiaoda.com/projects/supabase298428991113572352/functions/v1/serve_website?name=compress"
                },
                {
                    name: "Dislink 工具合集",
                    desc: "MIDI转mcpack/mcstructure、mcfunction转换、图片转mcfunction等",
                    url: "https://dislink.github.io"
                }
            ]
        },
        {
            ...
            items: [{
                ...
                },
                {
                    name: "NBT Studio",
                    desc: "NBT 数据编辑器",
                    url: "https://github.com/tryashtar/nbt-studio/releases"
                }
            ]
        },
        {
            ...
            items: [{
                name: "小舟工具箱",
                desc: "常用指令工具箱",
                url: "https://tool.lonzov.top"
            }, {
                name: "MC Toolbox",
                desc: "give/指令生成器，支持基岩版组件",
                url: "https://mctoolbox.net/zh-cn"
            }, {
                name: "CraftCMD",
                desc: "give/loot/execute 等命令构建器",
                url: "https://www.craftcmd.com/give"
            }, {
                name: "MineBuildr",
                desc: "指令/数据包/像素画工具集",
                url: "https://minebuildr.com/tools/command-generator"
            }, {
                name: "MCWorldTools",
                desc: "多版本命令生成器",
                url: "https://mcworldtools.com/tools/command-generator"
            }]
        },
        {
            sec: "结构与格式",
            ...
            items: [{
                    name: "4D皮肤模型",
                    desc: "4D 皮肤模型生成",
                    url: "https://fangkuaichaoge.github.io/"
                },
                {
                    name: "Schem To Schematic",
                    desc: "结构格式转换",
                    url: "https://schemtoschematic.app"
                },
                {
                    name: "MC Schematic",
                    desc: "格式转换与结构工具",
    ...
    function renderSites() {
        ...
        box.addEventListener("click", function(e) {
            ...
            if (!url) {
                var link = item.querySelector(".site-url");
            }
            if (o.classList.contains("copy")) {
                copyText(url || "");
            } else {
        ...
        box.querySelectorAll(".site-item").forEach(function(item) {
            item.addEventListener("click", function() {
                openExternalBrowser(item.getAttribute("data-url") || "");
            });
    ...
    window.openExternalBrowser = function(url) {
        ...
        var ok = false;
        try {
            ok = !!(window.open(url, "_blank", "noopener,noreferrer"));
        } catch (e) {
            ok = false;
        }
        if (!ok) {
